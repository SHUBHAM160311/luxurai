...
async def create_razorpay_order(body: CreateOrderRequest, request: Request):
    ...
    if not RAZORPAY_KEY_ID or not RAZORPAY_KEY_SECRET:
        ...
        fake_order_id = "order_dev_" + _new_id()
        await save_order(fake_order_id, user_id, body.pack_id, amount_inr, lc_amount, "razorpay_dev")
        return {
...
async def create_stripe_intent(body: StripeOrderRequest, request: Request):
    ...
    if not STRIPE_SECRET_KEY:
        # Dev mode
        return {
            "client_secret": "pi_dev_secret_placeholder",
            "lc_amount": lc_amount,
...
async def simulate_payment(request: Request, pack_id: str = "popular"):
    ...
    """
    if RAZORPAY_KEY_ID and not RAZORPAY_KEY_ID.startswith("rzp_test"):
